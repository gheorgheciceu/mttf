...
$(document).ready(
		function() {
			...
			$('.thumb-slider li a').click(
					function() {
						var url_img = $(this).attr('href');
						$(this).parents('.images-product').find('#large-img')
								.attr('href', url_img);
						$(this).parents('.images-product').find(
								'#large-img img').fadeOut(300).attr('src',
								url_img).fadeIn(300);
						return false;
					});
		});
...
jQuery(document).ready(function() {
	if ($(".wr-wrapper-v4 .wr-feature-product .products-slider").length) {
		$(".wr-wrapper-v4 .wr-feature-product .products-slider").owlCarousel({
			...
			slideSpeed : 500,
			stopOnHover : true,
			itemsCustom : [ [ 0, 1 ], [ 550, 2 ], [ 769, 3 ], [ 981, 4 ] ]
		});
	}
			...
			nav_container.waypoint({
				handler : function(event, direction) {
					if (direction == 'down') {
						...
						nav.stop().addClass("on-top").css("top",
								-nav.outerHeight()).animate({
							"top" : top_spacing
						});
					} else {
						nav_container.css({
						...
						nav.stop().removeClass("on-top").css("top",
								nav.outerHeight() + waypoint_offset).animate({
							"top" : ""
						});
					}
				},
		...
		function() {
			...
			$('.product-item').each(
					function(index, element) {
						$(this).find('.product-info').css('bottom',
								-($('.product-info').height()));
					});

			$('.list-btn').click(
					function() {
						$(this).parents('#content').find('ul.products')
								.removeClass('products-grid').addClass(
										'products-list');
					});
			...
			$('.grid-btn').click(
					function() {
						$(this).parents('#content').find('ul.products')
								.removeClass('products-list').addClass(
										'products-grid');
					});
			...
			$('.portfolio li').each(
					function(index, element) {
						$(this).find('.post-info').css('bottom',
								-($('.post-info').height()));
					});
			...
			$('.sidebar-menu > ul > li').click(
					function() {
						$(this).parent().find('.sub-menu').not(
								$(this).find('.sub-menu')).slideUp(300);
						$(this).find('.sub-menu').toggle(300);
						$(this).toggleClass("opend");
						$(this).parent().find('li').not($(this)).removeClass(
								'opend');
					});
			...
			$('.cart-page .remove').click(
					function() {
						$(this).parents('.cart-item').fadeOut(500).delay(10)
								.queue(function() {
									$(this).remove();
								});
						return false;
			...
			$('#accordion .action').click(
					function() {
						$(this).parents('.panel-group').find('.panel-content')
								.not($(this).parent().find('.panel-content'))
								.slideUp(300);
						$(this).parent().find('.panel-content').toggle(300);
						$(this).parent().toggleClass("opend");
						$(this).parent().parent().find('li').not(
			...
			$('.tab-control a').click(
					function() {
						...
						$(this).parent().find('a').not(this).removeClass(
								'active');
						$(this).addClass('active');

						$(this).parents('.tab-box').find('.tabs-content').not(
								selector).slideUp(300);
						$(this).parents('.tab-box').find(selector).slideDown(
								300);
				...
				tab_control.each(function(index, element) {
					...
					$(this).find('a').click(
							function() {
								...
								$(this).addClass('active');
								selector.parent().find('.home-tabs-content')
										.fadeOut(300);
			...
			if ($('.progress-bar').length) {
				$('.progress-bar').each(
						function(index, element) {
							$(this).css(
									'width',
									$(this).attr('aria-valuetransitiongoal')
											+ '%')
						});
			}
		...
		.ready(
				function(e) {
					if ($('.masonry-blog').length) {
						var $container = $('.masonry-blog'), $body = $('.container'), colW = 40, columns = null;
						...
						$(window).smartresize(
								...
								}).smartresize(); // trigger resize to set
													// container width
					}
				});
						...
						function() {
							...
							if ($(window).scrollTop() >= pos) {
								if ($(this).hasClass('intop')) {
									$(this).addClass("outtop").delay(delay)
											.queue(
													function() {
														$(this).removeClass(
																"intop").stop()
									...
									$(this).addClass("outleft").delay(delay)
											.queue(
													function() {
														$(this).removeClass(
																"inleft")
									...
									$(this).addClass("outright").delay(delay)
											.queue(
													function() {
														$(this).removeClass(
																"inright")
									...
									$(this).addClass("outbottom").delay(delay)
											.queue(
													function() {
														$(this).removeClass(
																"inbottom")
								...
								if ($(this).hasClass('outtop')) {
									$(this).removeClass('outtop').addClass(
											'intop');
								} else if ($(this).hasClass('outleft')) {
									$(this).removeClass('outleft').addClass(
											'inleft');
								} else if ($(this).hasClass('outright')) {
									$(this).removeClass('outright').addClass(
											'inright');
								} else if ($(this).hasClass('outbottom')) {
									$(this).removeClass('outbottom').addClass(
											'inbottom');
								}
							}
						});
